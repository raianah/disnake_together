...
class DisnakeTogether(AsyncObject):
    """
    ...
    async def __init__(self, token: str, *, debug: Optional[bool] = False):
        ...
        if isinstance(debug, bool):
            ...
            self.debug = False
            print('\033[93m'+"[WARN] (disnake-together) Debug parameter did not receive a bool object. Reverting to Debug = False."+'\033[0m') 
    ...
    async def create_link(self, voiceChannelID: Union[int,str], option: Union[int,str], *, max_age: Optional[int] = 0, max_uses: Optional[int] = 0) -> str:
        """
        Generates an invite link to a VC with the Disnake Party VC Feature.
        Parameters
        ----------
        ...
        option: Union[:class:`str`, :class:`int`]
            A option amongst the predefined choices (DisnakeTogether.default_choices) or a custom ID
        ...
        ----------
        :class:`str`
            Contains the disnake invite link which, upon clicked, starts the custom activity in the VC. 
        """
        ...
        if option and (str(option).lower().replace(" ", "") in defaultApplications.keys()):   
            ...
            if self.debug:
                print('\033[95m'+'\033[1m'+'[DEBUG] (disnake-together) Response Output:\n'+'\033[0m'+'HTTP RESPONSE CODE: '+str(resp_code)+'\n'+str(result))
